...
# gunicorn (unlike `runserver`) never serves static files itself, so
# WhiteNoise serves them straight out of app static dirs — no separate
# nginx/collectstatic step needed for a single-container deploy like this one.
STORAGES = {

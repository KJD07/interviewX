...
function MetricCard({ title, value, detail }: { title: string; value: string | number; detail?: string }) {
  return <div className="rounded-2xl p-4 sm:p-5 min-w-0" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}><p className="text-xs uppercase tracking-wider truncate" style={{ color: "var(--ink-faint)" }}>{title}</p><p className="mt-2 font-display text-xl sm:text-3xl font-semibold tabular-nums break-words" style={{ color: "var(--ink)" }}>{value}</p>{detail && <p className="mt-1 text-xs" style={{ color: "var(--ink-dim)" }}>{detail}</p>}</div>;
}
...
function Donut({ data }: { data: Record<string, number> }) {
  ...
  return <div className="flex flex-col sm:flex-row items-center gap-6"><div className="h-32 w-32 rounded-full shrink-0" style={{ background: `conic-gradient(${gradient})` }}><div className="m-5 h-22 w-22 rounded-full flex items-center justify-center" style={{ background: "var(--surface)" }}><span className="font-display font-semibold">{total}</span></div></div><div className="space-y-2 min-w-0">{Object.entries(data).map(([name, value], index) => <div key={name} className="flex items-center gap-2 text-xs"><span className="h-2 w-2 rounded-full shrink-0" style={{ background: colors[index % colors.length] }} /><span className="break-words" style={{ color: "var(--ink-dim)" }}>{displayName(name)}</span><strong className="shrink-0" style={{ color: "var(--ink)" }}>{value}</strong></div>)}</div></div>;
}
...
function InlineChildrenSection({ inline, groups, parentId }: {
  ...
  return <div className="mb-5 rounded border p-3" style={{ borderColor: "var(--border)", background: "var(--surface)" }}>
    ...
    <ul className="text-xs space-y-1">
      {rows?.map((row) => <li key={row.id} className="break-words" style={{ color: "var(--ink-dim)" }}>#{row.id} — {String(row[childModel.list_display[1] ?? childModel.list_display[0]] ?? row.id)}</li>)}
    </ul>
...
function AdminWorkspace() {
  ...
  return <div className="min-h-screen flex" style={{ background: "var(--page)" }}>
    ...
    <section className="flex-1 min-w-0">
      <header className="h-16 px-4 sm:px-8 flex items-center justify-between gap-3 border-b" style={{ borderColor: "var(--border)" }}><div className="flex items-center gap-3 min-w-0"><button className="md:hidden p-2 rounded shrink-0" aria-label="Open menu" onClick={() => setMobileOpen(true)} style={{ color: "var(--ink-dim)" }}>☰</button><div className="min-w-0"><p className="text-xs uppercase tracking-widest" style={{ color: "var(--ink-faint)" }}>Workspace</p><h1 className="font-display text-lg font-semibold truncate" style={{ color: "var(--ink)" }}>{selected?.label ?? "Admin"}</h1></div></div><div className="flex items-center gap-3 shrink-0"><span className="hidden sm:block text-xs" style={{ color: "var(--ink-faint)" }}>{user?.email}</span><button onClick={logout} className="text-xs font-medium hover:underline whitespace-nowrap" style={{ color: "var(--accent)" }}>Sign out</button></div></header>
      {showInsights ? <Insights onBack={() => setShowInsights(false)} /> : <main className="p-4 sm:p-8 max-w-[1400px]"><div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6"><p className="text-sm" style={{ color: "var(--ink-dim)" }}>{total} records</p><div className="flex flex-wrap gap-2 items-center">
        ...
        {selected?.actions.length ? <div className="flex flex-wrap gap-2 mb-4 items-center">{selected.actions.map((action) => <button key={action.name} onClick={() => runAction(action.name, action.label)} className="rounded px-3 py-1.5 text-xs font-medium text-left whitespace-normal break-words max-w-full sm:max-w-xs" style={{ border: "1px solid var(--border-mid)", color: "var(--ink-dim)" }}>{action.label}</button>)}<span className="text-xs break-words" style={{ color: "var(--ink-faint)" }}>{selectedIds.size ? `${selectedIds.size} selected` : "acts on all rows on this page unless you check some"}</span></div> : null}
        <div className="overflow-x-auto rounded border" style={{ background: "var(--surface)", borderColor: "var(--border)" }}><table className="w-full text-left text-sm"><thead><tr style={{ borderBottom: "1px solid var(--border)" }}><th className="px-4 py-3 w-8"><input type="checkbox" checked={rows.length > 0 && selectedIds.size === rows.length} onChange={toggleAll} style={{ accentColor: "var(--accent)" }} /></th>{selected?.list_display.map((field) => <th key={field} className="px-4 py-3 text-xs uppercase tracking-wider whitespace-nowrap" style={{ color: "var(--ink-faint)" }}>{displayName(field)}</th>)}<th /></tr></thead><tbody>{busy ? <tr><td colSpan={(selected?.list_display.length ?? 0) + 2} className="px-4 py-10 text-center" style={{ color: "var(--ink-faint)" }}>Loading...</td></tr> : rows.map((row) => <tr key={row.id} style={{ borderBottom: "1px solid var(--border)" }}><td className="px-4 py-3 align-top"><input type="checkbox" checked={selectedIds.has(row.id)} onChange={() => toggleRow(row.id)} style={{ accentColor: "var(--accent)" }} /></td>{selected?.list_display.map((field) => <td key={field} className="px-4 py-3 max-w-[240px] whitespace-normal break-words align-top" style={{ color: "var(--ink-dim)" }}>{String(row[`${field}_label`] ?? row[field] ?? "—")}</td>)}<td className="px-4 py-3 whitespace-nowrap text-right align-top">{selected?.can_change && <button onClick={() => openForm(row)} className="mr-3 text-xs font-medium hover:underline" style={{ color: "var(--accent)" }}>Edit</button>}{selected?.can_delete && <button onClick={() => remove(row)} className="text-xs hover:underline" style={{ color: "var(--danger)" }}>Delete</button>}</td></tr>)}</tbody></table>{!busy && !rows.length && <p className="px-4 py-10 text-center text-sm" style={{ color: "var(--ink-faint)" }}>No records found.</p>}</div><div className="flex justify-between mt-4 text-sm" style={{ color: "var(--ink-dim)" }}><button disabled={page === 1} onClick={() => setPage(page - 1)} className="disabled:opacity-30">Previous</button><span>Page {page}</span><button disabled={page * 25 >= total} onClick={() => setPage(page + 1)} className="disabled:opacity-30">Next</button></div>
      </main>}
    </section>
